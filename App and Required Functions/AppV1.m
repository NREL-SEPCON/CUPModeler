    ...
    methods (Access = private)
        ...
        function saveState(app)
            ...
            inputsToSave = struct('F', app.FlowRate.Value, ...
                                  ...
                                  'pulsePlot', guidata(app.UIAxesPulse), ...
        ...
        function loadState(app)
            ...
            pulsePlotData = inputsToSave.pulsePlot;
            ...
            if ~isempty(pulsePlotData)
                dataMidpoint = length(pulsePlotData)/2;
                X = pulsePlotData(1:dataMidpoint);
                Y = smooth(pulsePlotData(dataMidpoint+1:end), app.PulseSpan.Value)-app.PulseBaseline.Value;
                plot(app.UIAxesPulse, X, Y, 'linewidth', 2.0);
            end
            
            if ~isempty(fitPlotData)
                dataMidpoint = length(fitPlotData)/2;
                X = fitPlotData(1:dataMidpoint);
                Y = smooth(fitPlotData(dataMidpoint+1:end), app.PulseSpan.Value)-app.PulseBaseline.Value;
        ...
        function fitCoefficients(app)
            if height(app.PulseNList.Data) > 0
       ...
       function CUPPrediction(app)
            ...
            if contains(identifier, 'Export')
                ...
                if contains(identifier, 'Multi')
                    exportedPlot = sprintf('app.UIAxes' + ExportIdentifier + 'Position');
                    filename = strcat(filepath,'/',name,'_Position','.pdf');
                    exportgraphics(eval(exportedPlot), filename, 'ContentType', 'vector');
                end
            end
